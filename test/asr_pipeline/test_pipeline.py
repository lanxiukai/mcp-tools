...
def _get_test_audio(name: str) -> str | None:
    ...
    Resolves logical sample names to bundled fixtures under ``mcp-tool-test/``
    (which is git-ignored — present locally for testing, absent on fresh clones).

    Logical names recognized:
      - ``english_tech_speech.mp3``: an English single-speaker podcast clip
      - ``chinese_tech_speech.mp3``: a Chinese single-speaker speech clip
    Any other name is searched as a literal filename.
    """
    test_root = _REPO_ROOT / "mcp-tool-test"

    aliases: dict[str, list[Path]] = {
        "english_tech_speech.mp3": [
            test_root / "smoke-test" / "pipeline_smoke_test.mp3",
            test_root / "asr" / "podcast" / "en_single" / "greatinventors_14_wright_brothers.mp3",
        ],
        "chinese_tech_speech.mp3": [
            test_root / "asr" / "daily" / "zh_en_single" / "ZH-CN_U0004_S0_283.wav",
        ],
    }
    candidates: list[Path] = list(aliases.get(name, []))
    # Generic search: any subdir of mcp-tool-test/ whose tail equals *name*.
    candidates.extend([
        test_root / "asr" / "podcast" / "en_single" / name,
        test_root / "asr" / "daily" / "zh_en_single" / name,
        test_root / "smoke-test" / name,
        # Legacy fallback for older local layouts
        _REPO_ROOT / "mcp_test" / "audio" / name,
    ])
...
class TestDiarization:
    ...
    def test_mock_segments_structure(self):
        ...
        ), mock.patch.object(diarize.Pipeline, "from_pretrained") as m_fp:
...
class TestTranscription:
    ...
    def test_mock_combines_chunks(self):
        ...
        with mock.patch.object(
            transcribe,
            "_read_audio_mono_f32",
            return_value=(np.zeros(16000, dtype=np.float32), 16000),
        ), mock.patch("transcribe.Qwen3ASRModel") as m_model:
...
class TestMerge:
    ...
    def test_merge_empty_words(self):
        """Without word timestamps, preserve only the speaker timeline."""
        merge = self._import_merge()
        result = merge.merge_diarization_asr(self._spk_segs, [])
        assert len(result) == len(self._spk_segs)
        assert [seg["speaker"] for seg in result] == [
            seg["speaker"] for seg in self._spk_segs
        ]
        assert all(seg["text"] == "" for seg in result)
        assert all(seg["words"] == [] for seg in result)

    def test_no_timestamps_json_marks_text_attribution_unavailable(self):
        """Fast-mode JSON must not imply that text was assigned to speakers."""
        merge = self._import_merge()
        segs = merge.merge_diarization_asr(self._spk_segs, [])
        ...
        assert data["metadata"]["full_text"] == "Complete transcript."
        assert data["metadata"]["speaker_text_attribution"] is False
        assert data["metadata"]["speaker_timeline_available"] is True
...
class TestPipelineCLI:
    ...
    def test_num_speakers_must_be_positive(self, value):
        """An exact speaker count must be a positive integer."""
        with pytest.raises(SystemExit) as exc:
            self._pipeline_main("/fake.wav", "--num-speakers", value)
    ...
    def test_no_timestamps_txt_preserves_full_text_with_diarization(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir, \
                mock.patch("pipeline.preprocess.preprocess_audio", return_value="/fake.wav"), \
                mock.patch("pipeline.preprocess.get_audio_duration", return_value=8.0), \
                mock.patch.object(
                    _p._diarize_mod,
                    "run_diarization",
            ...
            txt = Path(tmpdir, "fake.txt").read_text(encoding="utf-8")
            data = json.loads(Path(tmpdir, "fake.json").read_text(encoding="utf-8"))
