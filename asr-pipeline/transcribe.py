...
# Resolve the ASR model via the shared model-source resolver.
REPO_DIR = Path(__file__).resolve().parent.parent
if str(REPO_DIR) not in sys.path:
    sys.path.insert(0, str(REPO_DIR))
from asr.model_source import (  # noqa: E402
...
def run_transcription(
    ...
    if return_timestamps:
        _fa = resolve_forced_aligner_source(None, REPO_DIR)
    ...
    model: Qwen3ASRModel = Qwen3ASRModel.from_pretrained(
        resolve_model_source(None, REPO_DIR),
        dtype=dtype,
        device_map=device,
        forced_aligner=_fa,
