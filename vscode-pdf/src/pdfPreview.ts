...
export class PdfPreview extends Disposable {
  ...
  public reveal(): void { this.webviewEditor.reveal(); }
      ...
      webviewEditor.webview.onDidReceiveMessage((message) => {
        switch (message.type) {
          ...
          case 'open-document-link': {
            if (typeof message.href !== 'string') break;
            void openDocumentLink(
              message.href, resource, message.resolvedByConverter === true, this.navigatePdf,
  ...
  private getWebviewContents(): string {
    ...
    const settings = {
      ...
      documentUri: this.resource.with({ scheme: 'file', authority: '', query: '', fragment: '' }).toString(),
      defaults: {
...
<script src="${resolveAsUri('lib', 'local-links.js')}"></script>
