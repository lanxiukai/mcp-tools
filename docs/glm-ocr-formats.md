...
| 工具 | 参数 | 说明 |
|---|---|---|---|
| `ocr_glm(file_path, output_format?, save_markdown?)` | `file_path`（必填）, `output_format`（可选，默认 `"markdown"`）, `save_markdown`（可选，默认 `true`） | 解析文档。小文件/图片同步解析；多页 PDF 自动异步提交+轮询。默认保存 `_ocr.md` 到源文件同目录 |
| `ocr_glm_submit(file_path)` | `file_path`（必填） | 异步提交文档，立即返回 `{job_id, total_pages}`。适合并行处理多个大 PDF |
| `ocr_glm_wait(job_id, max_wait?)` | `job_id`（必填）, `max_wait`（可选，默认 1800s） | 轮询等待异步任务完成并返回完整结果 |
| `ocr_glm_status(job_id?)` | `job_id`（可选） | 无参数：返回服务状态（模型、GPU 显存）。有 `job_id`：返回指定任务的进度 |

**自动唤醒**：首次调用 OCR 工具时，MCP Server 自动检测后端是否在线；离线则**非阻塞**后台启动（不等待），返回提示让 agent 30 秒后重试。后端无请求 30 秒后自动退出释放 GPU。
...
{
  "markdown": "# 第一章\n\n这是文档内容...\n\n| 列1 | 列2 |\n|---|---|\n| A | B |\n\n$$E=mc^2$$",
  "markdown_path": "/home/user/report_ocr.md",
  "_note": "Formulas are in LaTeX format (e.g., $E=mc^2$ for inline, $$...$$ for display). Tables are in Markdown table format."
}
```

默认 `save_markdown=True`，自动在同目录保存 Markdown 文件（PDF → `*_ocr.md`，图片 → `*.md`）。返回结果中包含 `markdown_path` 字段指向该文件。`save_markdown=False` 可关闭。
...
PDF 解析流程（`_predict_pdf_with_progress` 方法）：

```
PDF 文件
  → pymupdf (fitz) 逐页渲染
  → 300 DPI 位图
  → 逐页 OCR（每页独立调用 GLM-OCR，支持进度回调）
  → 多页拼接："\n\n---\n\n".join(all_markdown)
```
...
```
Agent 调用 ocr_glm("large.pdf")
  → MCP Server: POST /v1/ocr/submit → 立即返回 {job_id, total_pages}
  → MCP Server: 每 3 秒 GET /v1/ocr/jobs/{job_id} 轮询进度
  → 完成后 GET /v1/ocr/jobs/{job_id}/result → 返回完整 Markdown
  → Agent 收到结果（整个调用可能持续数分钟，但每次 HTTP 请求仅几秒）
```
...
| 像素格式 | RGB |
| 多页分隔符 | `\n\n---\n\n` |
| 任务超时 | 30 分钟（opencode.jsonc `"timeout": 1800000`） |
| MCP 调用模式 | 小文件/图片: 同步 HTTP；PDF: 异步 submit + 轮询 |
...
实测环境：RTX 4070 Ti 12GB，sdpa 注意力。逐页 300 DPI 渲染 + VLM 推理。

| 场景 | 实测时间 | 说明 |
|---|---|---|
| 单张图片（A4, 300 DPI） | 3~8 秒 | 取决于文本密度 |
| PDF（27 页，体检报告，含表格） | ~9 分钟 | ~20 秒/页 |
| PDF（100 页） | ~25-40 分钟 | 线性累加 |

> **MCP timeout 必须 ≥ 预期 OCR 时间**。配置 `"timeout": 1800000`（30 分钟）可覆盖大多数场景。超大型 PDF 建议用 `ocr_glm_submit` + `ocr_glm_wait` 分步操作。
