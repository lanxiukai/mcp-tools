...
本文档列出 5 个 MCP 工具的 API、配置、模型说明与性能数据。工具的使用方法、测试文件与冒烟测试见 [`docs/mcp-tools-testing.md`](mcp-tools-testing.md)。
...
纯 API 调用工具（无本地 GPU），通过 OpenRouter 使用 Qwen-3.7-Plus 的视觉能力。为纯文本模型（如 DeepSeek V4）提供 image-to-text 补充。

```python
# 通用图片分析
analyze_image("/home/user/screenshot.png")
analyze_image("/home/user/diagram.jpg", prompt="描述这个架构图中的组件和数据流")
...
# PDF 分析（逐页渲染为图片后调用 Qwen）
analyze_pdf("/home/user/report.pdf")
analyze_pdf("/home/user/slides.pdf", prompt="描述每页的图表配色和图片内容")
analyze_pdf("/home/user/long.pdf", max_pages=3)  # 只分析前 3 页
```

**模型**: Qwen-3.7-Plus（OpenRouter API，无需本地部署）
...
> `<YOUR-PYTHON>` 可为系统 Python（Python 3.10+，仅需 `mcp>=1.0.0`）。`OPENROUTER_API_KEY` 需在环境中或 opencode.jsonc 的 `env` 字段中设置。
...
**适用场景**:
- 截图理解（UI 布局/文字/问题定位）
- PDF 文档分析（保留颜色、图片、图表、布局）
- 图表数据提取（柱状图/折线图/饼图→结构化分析）
- 文档/幻灯片文字提取
- 为纯文本模型提供视觉上下文

> `analyze_pdf` 默认只分析前 5 页（可通过 `max_pages` 调整）。内部用 pymupdf 将 PDF 逐页渲染为 PNG，再调用 Qwen-3.7-Plus 进行视觉分析——因此能保留颜色、图片、图表等视觉信息，不是纯 OCR。若仅需文字提取，建议优先使用 `glm_ocr`（更快更便宜）。
